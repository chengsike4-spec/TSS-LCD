...
class ContextTransformerAE(nn.Module):
    def __init__(self, T, D, latent_dim, nhead=2, num_layers=2):
        super().__init__()
        self.T, self.D = T, D
        # 时间分支
...
#         self.freq_local_encoder = TransformerEncoder(input_dim, num_heads, num_layers, dim_feedforward, dropout)
...
#             loc = self.freq_local_encoder(loc)   # (B, 250, 750)
...
if __name__ == "__main__":
    ...
    test_loader = DataLoader(torch.utils.data.Subset(dataset, list(range(split, N))), batch_size=128,
                             shuffle=False)
    model = ContextTransformerAE(T=50, D=750, latent_dim=16)
    # ae_ctx = ContextTransformerAE(seq_len=50, input_dim=750, latent_dim=16)
    ae_ctx = train_autoencoder(model, train_loader, device, 300)
